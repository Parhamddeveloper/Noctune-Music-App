...
export default function PlayListInfoPage() {
  ...
  if (!playlist)
    return (
      <h2 className="text-2xl text-(--color-text) font-bold text-center mt-6">
        Playlist not found
      </h2>
    );
    ...
    <>
        ...
        <div className="flex items-center gap-x-4 min-w-0">
          <Link to={"/library"}>
            <button
              ...
              className="text-(--color-text)/80 size-11 bg-(--color-text)/8 backdrop-blur-2xl grid place-items-center rounded-full border border-(--color-text)/10 shrink-0 hover:bg-(--color-text)/20 transition-colors"
            >
              <ArrowLeft size={19} />
            </button>
          </Link>
          <div className="flex flex-col gap-y-2 min-w-0">
            <p className="text-(--color-text)/45">playlist</p>
            <h1 className="text-(--color-text) text-xl font-bold truncate">Discover </h1>
          </div>
        </div>
        <div className="flex gap-x-3">
          <button
            aria-label="notification button"
            className="text-(--color-text)/80 size-11 bg-(--color-text)/8 backdrop-blur-2xl grid place-items-center rounded-full border border-(--color-text)/10 hover:bg-(--color-text)/20 transition-colors"
      ...
      <div className=" border border-(--color-text)/15 bg-linear-to-br from-(--color-surface)/25 via-fuchsia-500/10 to-cyan-400/10 p-5 shadow-2xl shadow-violet-950/30 backdrop-blur-3xl text-(--color-text) rounded-3xl mt-5">
        <div className="w-4/6 md:w-full flex flex-col items-start">
          <div className="grid text-(--color-primary) size-15 bg-white/8 backdrop-blur-2xl place-items-center rounded-2xl border border-violet-300/30">
            <Music size={26} />
          </div>
          <p className="text-lg font-bold mt-5">{playlist.title}</p>
          <p className="text-(--color-text)/50 mt-2">{playlist.songs.length} songs</p>
        </div>
      </div>
