        ...
        {navLinks.map((LinkItem) => {
          ...
          return (
            <NavLink
              ...
              to={LinkItem.href}
              className={({ isActive }) =>
                `flex flex-col items-center gap-1 rounded-2xl px-2 py-2 text-xs transition min-w-14 ${isActive ? "bg-white/15 text-violet-300 shadow-inner shadow-white/10" : "text-white/45 hover:text-white"}`
              }
            >
              <Icon />
              {LinkItem.title}
